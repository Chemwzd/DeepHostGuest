...
Because general ML potentials require atom types and atom coordinates, this workflow uses the host `.mol` conformer rather than the host `.ply` surface mesh as the scoring input. For UMA, `task_name="omol"` is a good default for finite molecular complexes; choose another FAIRChem task such as `"omc"` only when it better represents your host-guest system. The output files contain the predicted low-energy complex and the corresponding guest conformation.
