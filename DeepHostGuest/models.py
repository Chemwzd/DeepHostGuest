...
class ResBlock(nn.Module):
    def __init__(self, in_channels, dropout_rate=0.15):
        ...
        self.conv = MetaLayer(edge_model=EdgeModel(in_channels // 4), node_model=NodeModel(in_channels // 4),
                              global_model=None)
...
class DeepDock(nn.Module):
    def __init__(self, ligand_model, target_model, hidden_dim, n_gaussians, dropout_rate=0.15,
        ...
        self.MLP = nn.Sequential(nn.Linear(256, hidden_dim), nn.BatchNorm1d(hidden_dim), nn.ELU(),
    ...
    def compute_euclidean_distances_matrix(self, X, Y):
        ...
        dists = (-2 * torch.bmm(X, Y.permute(0, 2, 1)) +
                 torch.sum(Y ** 2, axis=-1).unsqueeze(1) + torch.sum(X ** 2,
                                                                     axis=-1).unsqueeze(
                    -1))
        return dists ** 0.5
